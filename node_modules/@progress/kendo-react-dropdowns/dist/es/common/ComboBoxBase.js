...
import { getItemText, getItemValue, getItemIndex, itemIndexStartsWith, getItemIndexByText } from '../common/util';
/**
 * @hidden
 */
var ComboBoxBase = /** @class */ (function () {
    ...
    ComboBoxBase.prototype.componentWillUpdate = function (nextProps, nextState) {
        ...
        if (indexChanged) {
            ...
            this.dropdown.value = getItemValue(newItem, nextProps.valueField);
            this.dropdown.text = getItemText(newItem, nextProps.textField);
    ...
    ComboBoxBase.prototype.onNavigate = function (event) {
        ...
        var index = getItemIndex(component.props, component.value);
        ...
        if (currentIndex < 0 && text && (event.keyCode === 38 /* up */ || event.keyCode === 40 /* down */)) {
            var focusedIndex = itemIndexStartsWith(text, data, textField);
            if (focusedIndex >= 0) {
    ...
    ComboBoxBase.prototype.valueToRender = function (state, props) {
        ...
        if (hasFilter) {
            ...
            var typedTextMatchIndex = getItemIndexByText(data, typedText, textField);
            if (typedTextMatchIndex === -1) {
                return { value: null, text: typedText };
            }
            else {
                return { value: getItemValue(data[typedTextMatchIndex], valueField), text: typedText };
            }
        }
    ...
    ComboBoxBase.prototype.getFocusedIndex = function (text) {
        ...
        if (opened) {
            if (focusedIndex !== undefined) {
                ...
                return itemIndexStartsWith(text, data, textField);
            }
            else {
                return (skip === 0 ? 0 : -1);
            }
    ...
    ComboBoxBase.prototype.applyValueOnEnter = function (value, state) {
        ...
        if (suggest && suggestedText) { // item is suggested
            ...
            newValue = getItemValue(data[valueIndex], valueField);
            newText = getItemText(data[valueIndex], textField);
        }
        else {
    ...
    ComboBoxBase.prototype.applyValueOnRejectSuggestions = function (value, state) {
        ...
        var valueIndex = getItemIndexByText(data, value, textField, true);
        var itemSelected = valueIndex !== -1;
        var newValue = undefined;
        var newText = '';
        if (component.state.suggestedText) {
    ...
    ComboBoxBase.prototype.selectFocusedItem = function (value, state) {
        ...
        var focusedItemIndex = itemIndexStartsWith(value, data, textField);
    ...
    ComboBoxBase.prototype.suggestValue = function (value, state) {
        ...
        if (component.props.suggest && value) {
            ...
            if (suggestedItem) {
                state.data.suggestedText = getItemText(suggestedItem, textField);
            }
        }
    };
    // Common rendering
    ...
    ComboBoxBase.prototype.renderList = function () {
        ...
        return (React.createElement(List, { id: dropdown.listBoxId, show: opened, data: data, focusedIndex: focusedIndex, value: value, textField: textField, valueField: valueField, optionPrefix: dropdown.optionPrefix, listRef: function (element) {
                if (vs.enabled) {
                    vs.list = element;
                }
    ...
    ComboBoxBase.propTypes = __assign({}, DropDownBase.propTypes, { suggest: PropTypes.bool, placeholder: PropTypes.string });
    ComboBoxBase.defaultProps = __assign({}, DropDownBase.defaultProps);
    return ComboBoxBase;
}());
